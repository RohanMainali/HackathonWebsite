...
export default async function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main-content">
    <section className="case-hero"><Container><div className="case-hero__meta"><span>{project.category}</span><span>{project.date ?? project.year}</span></div><h1>{project.title}</h1><p>{project.summary}</p></Container><div className="container"><MediaFrame {...project.coverImage} aspect="wide" priority /></div></section>
    <section className="section case-overview"><Container><div><p className="eyebrow">EVENT OVERVIEW</p><h2>A complete participant journey, built around a clear purpose.</h2>{project.source && <a className="text-link" href={project.source.href} target="_blank" rel="noreferrer">Source: {project.source.label} <span aria-hidden="true">↗</span></a>}</div><dl>{[["Event type", project.category], ["Format", project.format], ["Duration", project.duration], ["Audience", project.audience], ["Location", project.location], [project.date ? "Event date" : "Year", project.date ?? project.year]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></Container></section>
